...
HloInstruction* Truncate(HloInstruction* f32_param) {
  // Cast to int32 first, then zero out the high bits. Then cast back to bf16.
  auto u32_shape = f32_param->shape();
  u32_shape.set_element_type(PrimitiveType::U32);
  auto u32_param = f32_param->AddInstruction(
  ...
  auto u32_mask = u32_param->AddInstruction(
      HloInstruction::CreateBroadcast(u32_shape, mask_constant, {}));
  auto masked_u32 = u32_param->AddInstruction(HloInstruction::CreateBinary(
...
HloInstruction* SubAndRoundToBF16(HloInstruction* instr, HloInstruction* high) {
  auto sub = instr->AddInstruction(HloInstruction::CreateBinary(
...
std::pair<HloInstruction*, HloInstruction*> Split(HloInstruction* f32_param) {
  ...
  auto bf16_shape = high_f32->shape();
